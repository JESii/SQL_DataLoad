...
-- Testing
/*
select *  FROM @remittance MR
  JOIN mors_contract MCT ON MR.pos_authorization_no = MCT.pos_authorization_number
  JOIN mors_client MC ON MCT.BTRS_client_number = MC.BTRS_client_number
  --WHERE MC.uci_number IS NULL
  order by consumer_name
select * from mors_client MC
	where uci_number is null
	order by MC.BTRS_client_name  
*/
select DISTINCT MR.*, MC.BTRS_client_name, MC.BTRS_client_number, MC.id
  FROM @remittance MR
  JOIN mors_contract MCT ON MR.pos_authorization_no = MCT.pos_authorization_number
  JOIN mors_client MC ON MCT.BTRS_client_number = MC.BTRS_client_number
  WHERE MC.uci_number IS NULL
